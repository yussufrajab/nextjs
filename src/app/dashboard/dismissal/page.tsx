...
const initialMockPendingDismissalRequests: MockPendingDismissalRequest[] = [
  {
    ...
    reasonSummary:
      'Failure to meet performance standards during probation period.',
...
export default function DismissalPage() {
  ...
  const [appraisalFormFile, setAppraisalFormFile] = useState<FileList | null>(
    null
  );
  const [supportingDocumentsFile, setSupportingDocumentsFile] =
    useState<FileList | null>(null);

  const [warningLettersFile, setWarningLettersFile] = useState<FileList | null>(
    null
  );
  const [employeeExplanationLetterFile, setEmployeeExplanationLetterFile] =
    useState<FileList | null>(null);
  const [suspensionLetterFile, setSuspensionLetterFile] =
    useState<FileList | null>(null);
  const [summonNoticeFile, setSummonNoticeFile] = useState<FileList | null>(
    null
  );
  const [
    investigationCommitteeReportFile,
    setInvestigationCommitteeReportFile,
  ] = useState<FileList | null>(null);
  const [otherAdditionalDocumentsFile, setOtherAdditionalDocumentsFile] =
    useState<FileList | null>(null);

  const [letterOfRequestFile, setLetterOfRequestFile] =
    useState<FileList | null>(null);
  const [minProposedDate, setMinProposedDate] = useState('');

  const [pendingRequests, setPendingRequests] = useState<
    MockPendingDismissalRequest[]
  >(initialMockPendingDismissalRequests);
  const [selectedRequest, setSelectedRequest] =
    useState<MockPendingDismissalRequest | null>(null);
  ...
  const refreshRequests = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Could not refresh requests.',
        variant: 'destructive',
      });
    } finally {
  ...
  const isDismissalAllowed =
    employeeDetails && employeeDetails.status === 'On Probation';
  ...
  const handleFetchEmployeeDetails = async () => {
    if (!zanId) {
      toast({
        title: 'ZanID Required',
        description: "Please enter an employee's ZanID.",
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (foundEmployee.status !== 'On Probation') {
        ...
        toast({
          ...
          description: `Details for ${foundEmployee.name} loaded.`,
        });
      }
    } catch (error: any) {
      toast({
        title: 'Employee Not Found',
        description: error.message || `No employee found with ZanID: ${zanId}.`,
        variant: 'destructive',
      });
    } finally {
  ...
  const handleSubmitDismissalRequest = () => {
    ...
    if (!isDismissalAllowed) {
      toast({
        title: 'Submission Error',
        description: `Dismissal is only applicable to employees 'On Probation'. This employee is '${employeeDetails.status}'.`,
        variant: 'destructive',
        duration: 7000,
      });
      return;
    ...
    if (!reasonDismissal || !proposedDateDismissal) {
      toast({
        title: 'Submission Error',
        description: 'Reason for Dismissal and Proposed Date are required.',
        variant: 'destructive',
      });
      return;
    }
    if (!letterOfRequestFile) {
      toast({
        title: 'Submission Error',
        description:
          'Letter of Request is missing. Please upload the PDF document.',
        variant: 'destructive',
      });
      return;
    }

    const checkPdf = (fileList: FileList | null) =>
      !fileList || (fileList[0] && fileList[0].type === 'application/pdf');

    if (!checkPdf(appraisalFormFile)) {
      toast({
        title: 'Submission Error',
        description: 'Appraisal Form must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (supportingDocumentsFile && !checkPdf(supportingDocumentsFile)) {
      toast({
        title: 'Submission Error',
        description: 'Supporting Documents (general) must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (
      letterOfRequestFile &&
      letterOfRequestFile[0].type !== 'application/pdf'
    ) {
      toast({
        title: 'Submission Error',
        description: 'Letter of Request must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (warningLettersFile && !checkPdf(warningLettersFile)) {
      toast({
        title: 'Submission Error',
        description: 'Warning Letter(s) must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (
      employeeExplanationLetterFile &&
      !checkPdf(employeeExplanationLetterFile)
    ) {
      toast({
        title: 'Submission Error',
        description: 'Employee Explanation Letter must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (suspensionLetterFile && !checkPdf(suspensionLetterFile)) {
      toast({
        title: 'Submission Error',
        description: 'Suspension Letter must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (summonNoticeFile && !checkPdf(summonNoticeFile)) {
      toast({
        title: 'Submission Error',
        description: 'Summon Notice must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (
      investigationCommitteeReportFile &&
      !checkPdf(investigationCommitteeReportFile)
    ) {
      toast({
        title: 'Submission Error',
        description: 'Investigation Committee Report must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    if (
      otherAdditionalDocumentsFile &&
      !checkPdf(otherAdditionalDocumentsFile)
    ) {
      toast({
        title: 'Submission Error',
        description: 'Other Additional Documents must be a PDF file.',
        variant: 'destructive',
      });
      return;
    }
    ...
    const documentsList = ['Letter of Request'];
    if (appraisalFormFile) documentsList.push('Form for Appraisal');
    if (supportingDocumentsFile)
      documentsList.push('Supporting Documents (General)');
    if (warningLettersFile) documentsList.push('Warning Letter(s)');
    if (employeeExplanationLetterFile)
      documentsList.push('Employee Explanation Letter');
    ...
    if (investigationCommitteeReportFile)
      documentsList.push('Investigation Committee Report');
    if (otherAdditionalDocumentsFile)
      documentsList.push('Other Additional Document(s)');
    ...
    setTimeout(() => {
      ...
      toast({
        title: 'Dismissal Request Submitted',
  ...
  const handleCommissionDecision = (
    ...
    if (toastMessage) {
      toast({
        title: `Commission Decision: ${decision === 'approved' ? 'Approved' : 'Rejected'}`,
        description: toastMessage,
      });
    }
  };
  ...
  return (
    <div>
      <PageHeader
        title="Dismissal"
        description="Process employee dismissals for unconfirmed employees."
      />
      {role === ROLES.HRO && (
        <Card className="mb-6 shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Enter ZanID to fetch employee details, then complete the dismissal
              form. All documents must be PDF.
            </CardDescription>
          </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                <Input
                  ...
                  disabled={isFetchingEmployee || isSubmitting}
                />
                <Button
                  onClick={handleFetchEmployeeDetails}
                  disabled={isFetchingEmployee || !zanId || isSubmitting}
                >
                  {isFetchingEmployee ? (
              ...
              <div className="space-y-6 pt-2">
                  ...
                  <div className="p-4 rounded-md border bg-secondary/20 space-y-3 shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3 text-sm">
                      ...
                      <div className="md:col-span-2 lg:col-span-3">
                        ...
                        <p
                          className={`font-semibold ${isDismissalAllowed ? 'text-green-600' : 'text-red-600'}`}
                        >
                ...
                {!isDismissalAllowed && (
                  <div className="flex items-center p-4 mt-4 text-sm text-destructive border border-destructive/50 rounded-md bg-destructive/10">
                    ...
                    <span>
                      Dismissal is only applicable to employees 'On Probation'.
                      This employee is '{employeeDetails.status}'. Form
                ...
                <div
                  className={`space-y-4 ${!isDismissalAllowed ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <h3 className="text-lg font-medium text-foreground">
                    Dismissal Details &amp; Documents
                  </h3>
                  <div>
                    <Label htmlFor="reasonDismissal">
                      Reason for Dismissal
                    </Label>
                    <Textarea
                      id="reasonDismissal"
                      placeholder="Clearly state the grounds for dismissal"
                      value={reasonDismissal}
                      onChange={(e) => setReasonDismissal(e.target.value)}
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  <div>
                    <Label
                      htmlFor="proposedDateDismissal"
                      className="flex items-center"
                    >
                      <CalendarDays className="mr-2 h-4 w-4 text-primary" />
                      Proposed Date of Dismissal
                    </Label>
                    <Input
                      id="proposedDateDismissal"
                      type="date"
                      value={proposedDateDismissal}
                      onChange={(e) => setProposedDateDismissal(e.target.value)}
                      disabled={!isDismissalAllowed || isSubmitting}
                      min={minProposedDate}
                    />
                  </div>
                  ...
                  <div>
                    <Label
                      htmlFor="letterOfRequestFileDismissal"
                      className="flex items-center"
                    >
                      <FileText className="mr-2 h-4 w-4 text-primary" />
                      Upload Letter of Request (Required)
                    </Label>
                    <Input
                      id="letterOfRequestFileDismissal"
                      type="file"
                      onChange={(e) => setLetterOfRequestFile(e.target.files)}
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  ...
                  <h4 className="text-md font-medium text-foreground pt-2">
                    Upload Optional Supporting Documents (PDF Only)
                  </h4>
                  <div>
                    <Label
                      htmlFor="appraisalFormFile"
                      className="flex items-center"
                    >
                      <ClipboardCheck className="mr-2 h-4 w-4 text-primary" />
                      Upload Form for Appraisal
                    </Label>
                    <Input
                      id="appraisalFormFile"
                      type="file"
                      onChange={(e) => setAppraisalFormFile(e.target.files)}
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      id="warningLettersFile"
                      type="file"
                      onChange={(e) => setWarningLettersFile(e.target.files)}
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      id="employeeExplanationLetterFile"
                      type="file"
                      onChange={(e) =>
                        setEmployeeExplanationLetterFile(e.target.files)
                      }
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  ...
                  <div>
                    ...
                    <Input
                      id="suspensionLetterFile"
                      type="file"
                      onChange={(e) => setSuspensionLetterFile(e.target.files)}
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      id="summonNoticeFile"
                      type="file"
                      onChange={(e) => setSummonNoticeFile(e.target.files)}
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  </div>
                  <div>
                    <Label
                      htmlFor="investigationCommitteeReportFile"
                      className="flex items-center"
                    >
                      <FileText className="mr-2 h-4 w-4 text-primary" />
                      Upload Investigation Committee Report
                    </Label>
                    <Input
                      id="investigationCommitteeReportFile"
                      type="file"
                      onChange={(e) =>
                        setInvestigationCommitteeReportFile(e.target.files)
                      }
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  ...
                  <div>
                    <Label
                      htmlFor="supportingDocumentsFile"
                      className="flex items-center"
                    >
                    ...
                    <Input
                      ...
                      type="file"
                      onChange={(e) =>
                        setSupportingDocumentsFile(e.target.files)
                      }
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
                  ...
                  <div>
                    ...
                    <Input
                      id="otherAdditionalDocumentsFile"
                      type="file"
                      onChange={(e) =>
                        setOtherAdditionalDocumentsFile(e.target.files)
                      }
                      accept=".pdf"
                      disabled={!isDismissalAllowed || isSubmitting}
                    />
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Review, approve, or reject pending dismissal requests.
                </CardDescription>
              </div>
              <Button
                variant="outline"
          ...
          <CardContent>
            {pendingRequests.filter(
              (req) =>
                ...
                (role === ROLES.HHRMD &&
                  req.status === 'Pending HHRMD Review') ||
                req.status ===
                  'Request Received – Awaiting Commission Decision' ||
                req.status.startsWith('Rejected by') ||
                req.status.startsWith('Approved by Commission') ||
                req.status.startsWith('Rejected by Commission')
            ).length > 0 ? (
              pendingRequests
                .filter(
                  (req) =>
                ...
                .map((request) => (
                  ...
                  >
                    <h3 className="font-semibold text-base">
                      Dismissal for: {request.employeeName} (ZanID:{' '}
                      {request.zanId})
                    </h3>
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {request.proposedDate
                        ? format(parseISO(request.proposedDate), 'PPP')
                        : 'N/A'}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Submitted:{' '}
                      {request.submissionDate
                        ? format(parseISO(request.submissionDate), 'PPP')
                        : 'N/A'}{' '}
                      by{' '}
                      {(() => {
                        ...
                        if (
                          ...
                          return (submitter as { name: string }).name;
                    ...
                    <div className="mt-3 pt-3 border-t flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                      ...
                      {request.reviewStage === 'initial' &&
                        request.status.startsWith(`Pending ${role} Review`) && (
                          <>
                            <Button
                              size="sm"
                              onClick={() =>
              ...
              <p className="text-muted-foreground">
                No dismissal requests pending your review.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <strong>{selectedRequest.employeeName}</strong> (ZanID:{' '}
                ...
                <div className="grid grid-cols-3 items-center gap-x-4 gap-y-1">
                  ...
                  <p className="col-span-2 font-medium text-foreground">
                    {selectedRequest.employmentDate
                      ? format(parseISO(selectedRequest.employmentDate), 'PPP')
                      : 'N/A'}
                  </p>
                </div>
                <div className="grid grid-cols-3 items-center gap-x-4 gap-y-1">
                  ...
                  <p className="col-span-2 font-medium text-foreground">
                    {selectedRequest.dateOfBirth
                      ? format(parseISO(selectedRequest.dateOfBirth), 'PPP')
                      : 'N/A'}
                  </p>
                </div>
                <div className="grid grid-cols-3 items-center gap-x-4 gap-y-1">
                  ...
                  <p className="col-span-2 font-medium text-foreground">
                    {(() => {
                      ...
                      if (
                        ...
                        return (inst as { name: string }).name;
                ...
                <div className="grid grid-cols-3 items-center gap-x-4 gap-y-2">
                  ...
                  <p className="col-span-2">
                    {selectedRequest.proposedDate
                      ? format(parseISO(selectedRequest.proposedDate), 'PPP')
                      : 'N/A'}
                  </p>
                </div>
                <div className="grid grid-cols-3 items-center gap-x-4 gap-y-2">
                  ...
                  <p className="col-span-2">
                    {selectedRequest.submissionDate
                      ? format(parseISO(selectedRequest.submissionDate), 'PPP')
                      : 'N/A'}{' '}
                    by{' '}
                    {(() => {
      ...
      {currentRequestToAction && (
        ...
        >
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Reject Dismissal Request: {currentRequestToAction.id}
              </DialogTitle>
              <DialogDescription>
                Please provide the reason for rejecting the dismissal request
                for <strong>{currentRequestToAction.employeeName}</strong>. This
