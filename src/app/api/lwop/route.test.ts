...
const mockLwopFindUnique = vi.fn();
const mockLwopUpdate = vi.fn();
...
vi.mock('@/lib/db', () => ({
  db: {
    ...
    lwopRequest: {
      findUnique: (...a: any[]) => mockLwopFindUnique(...a),
      update: (...a: any[]) => mockLwopUpdate(...a),
    },
...
vi.mock('@/lib/notifications', () => ({
  ...
  NotificationTemplates: {
    lwopHrrpApproved: () => ({ message: 'm', link: 'l' }),
    lwopHrrpRejected: () => ({ message: 'm', link: 'l' }),
    lwopPendingHrrpReview: () => ({ message: 'm', link: 'l' }),
  },
}));
...
function authedRequest(body: unknown): NextRequest {
  return new NextRequest('http://localhost:9002/api/lwop', {
    method: 'PATCH',
    headers: {
...
describe('PATCH /api/lwop — Commission workflow (parity with promotion)', () => {
  beforeEach(() => {
    mockValidateSession.mockReset();
    mockUserFindUnique.mockReset();
    mockLwopFindUnique.mockReset();
    mockLwopUpdate.mockReset();
    ...
    mockLwopFindUnique.mockResolvedValue({
      id: 'req-1',
      employeeId: 'emp-1',
    ...
    mockLwopUpdate.mockResolvedValue({
      id: 'req-1',
      employeeId: 'emp-1',
      submittedById: 'hro-1',
      ...
      Employee: { id: 'emp-1', name: 'Emp', zanId: 'Z1', institutionId: 'inst-1' },
      User_LwopRequest_submittedByIdToUser: { id: 'hro-1', name: 'HRO', username: 'hro-user' },
      User_LwopRequest_reviewedByIdToUser: null,
      User_LwopRequest_hrrpReviewedByToUser: null,
    });
  });
  ...
  it('persists reviewStage="commission_review" when HHRMD forwards to the Commission', async () => {
    ...
    const updateArgs = mockLwopUpdate.mock.calls[0][0];
  ...
  it('completes the workflow and updates the employee status on Commission approval', async () => {
    ...
    const res = await PATCH(
      authedRequest({
        ...
        reviewedById: HHRMD_USER.id,
        commissionLetterKey: 'lwop/commission-letters/letter.pdf',
    ...
    // Commission approval must flip the employee to "On LWOP" status.
    ...
    expect(employeeArgs.data.status).toBe('On LWOP');
  });
