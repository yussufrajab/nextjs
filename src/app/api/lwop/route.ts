...
async function POSTHandler(req: Request) {
    ...
    if (isHRRP) {
      ...
      const notification = NotificationTemplates.lwopHrrpApproved(
      ...
      const notification = NotificationTemplates.lwopPendingHrrpReview(
        lwopRequest.Employee.name,
        lwopRequest.id
      );
...
async function PATCHHandler(req: Request) {
    ...
    if (!existingRequest) {
      return NextResponse.json(
        { success: false, message: 'LWOP request not found' },
        { status: 404 }
      );
    }
    ...
    const isCommissionDecision = updateData.reviewedById !== undefined && !isHrrpAction && !isResubmission && (updateData.status === 'Approved by Commission' || updateData.status === 'Rejected by Commission - Request Concluded');
    const isInitialReviewAction = updateData.reviewedById !== undefined && !isHrrpAction && !isCommissionDecision && !isResubmission;
    ...
    // FSM in lwop/[id]/route.ts, extended to handle the variable
    ...
    if (isHrrpApproval) {
      const hrrpNotification = NotificationTemplates.lwopHrrpApproved(
    ...
    if (isHrrpRejection) {
      const rejectionNotification = NotificationTemplates.lwopHrrpRejected(
        updatedRequest.Employee?.name || 'Unknown',
        id,
