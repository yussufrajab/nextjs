...
const complaintSchema = z.object({
  ...
  subject: z
    .string()
    .min(5, 'Kichwa lazima kiwe na angalau herufi 5.')
    .max(100, 'Kichwa lazima kiwe na herufi 100 au chini yake.'),
  complaintText: z
    .string()
    .min(20, 'Maelezo ya malalamiko lazima yawe na angalau herufi 20.'),
...
export default function ComplaintsPage() {
  ...
  const [officerAttachmentFile, setOfficerAttachmentFile] =
    useState<string>('');
  ...
  const [isCommissionDecisionModalOpen, setIsCommissionDecisionModalOpen] =
    useState(false);
  const [
    selectedComplaintForCommissionDecision,
    setSelectedComplaintForCommissionDecision,
  ] = useState<SubmittedComplaint | null>(null);
  const [commissionDecision, setCommissionDecision] = useState('');
  const [commissionDecisionType, setCommissionDecisionType] = useState<
    'resolved' | 'rejected'
  >('resolved');
  ...
  const fetchComplaints = async (isRefresh = false, page = currentPage) => {
    ...
    try {
      ...
      if (isRefresh) {
        toast({
          title: 'Imesasishwa',
          description: 'Malalamiko yamesasishwa.',
          duration: 2000,
        });
      }
    } catch (error) {
      toast({
        title: 'Hitilafu',
        description: 'Imeshindwa kupakia malalamiko.',
        variant: 'destructive',
      });
    } finally {
      if (isRefresh) {
  ...
  useEffect(() => {
    fetchComplaints();
  }, [user, role, currentPage, statusFilter]);

  const handleStandardizeComplaint = async () => {
    const complaintText = form.getValues('complaintText');
    if (!complaintText) {
      toast({
        title: 'Hitilafu ya Kuandika Upya',
        description: 'Tafadhali ingiza maelezo ya lalamiko lako kwanza.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleRejectionSubmit = async () => {
    if (!currentRequestToAction || !rejectionReasonInput.trim()) {
      toast({
        title: 'Hitilafu ya Kukataa',
        description: 'Sababu ya kukataa inahitajika.',
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: 'Lalamiko Limekataliwa',
      description: `Lalamiko ${id} la ${employeeName} limekataliwa.`,
      variant: 'destructive',
    });
    ...
    try {
      ...
      if (!updated) {
        ...
        toast({
          title: 'Hitilafu',
          description: 'Imeshindwa kukataa lalamiko.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleOfficerSubmitLegacyAction = async () => {
    if (!selectedComplaint || !actionType || !officerActionComment.trim()) {
      toast({
        title: 'Hitilafu ya Hatua',
        description: 'Maoni ya afisa yanahitajika.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (!updated) {
        ...
        toast({
          title: 'Hitilafu',
          description: 'Imeshindwa kuchukua hatua.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleEmployeeConfirmOutcome = async (complaintId: string) => {
    ...
    // Show immediate feedback
    toast({
      title: 'Thibitisho Limekamilika',
      description: 'Asante kwa kuthibitisha. Lalamiko limekamilika.',
    });
    ...
    try {
      ...
      if (!updated) {
        ...
        toast({
          title: 'Hitilafu',
          description: 'Imeshindwa kuthibitisha.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleProvideAdditionalInfo = async () => {
    if (!selectedComplaintForInfo || !additionalInfo.trim()) {
      toast({
        title: 'Hitilafu',
        description: 'Tafadhali toa maelezo ya ziada yaliyoombwa.',
        variant: 'destructive',
      });
      return;
    }
    ...
    // Show immediate feedback
    toast({
      title: 'Maelezo Yametolewa',
      description: 'Maelezo yako ya ziada yamewasilishwa kwa ukaguzi.',
    });
    ...
    try {
      ...
      if (!updated) {
        ...
        toast({
          title: 'Hitilafu',
          description: 'Imeshindwa kutoa maelezo.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleResubmitComplaint = async (data: ComplaintFormValues) => {
    ...
    // Show immediate feedback
    toast({
      title: 'Lalamiko Limewasilishwa Upya',
      description:
        'Lalamiko lako limerekebihwa na kuwasilishwa upya kwa ukaguzi.',
    });
    ...
    try {
      ...
      const updated = await handleUpdateComplaint(
        selectedComplaintForResubmission.id,
        payload
      );
      if (!updated) {
        // Revert optimistic update on failure
        setComplaints(complaints);
        toast({
          title: 'Kushindwa Kuwasilisha',
          description: 'Imeshindwa kuwasilisha upya lalamiko.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const handleCommissionDecision = async () => {
    if (!selectedComplaintForCommissionDecision || !commissionDecision.trim()) {
      toast({
        title: 'Hitilafu',
        description: 'Tafadhali toa maamuzi ya tume.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const updated = await handleUpdateComplaint(
        selectedComplaintForCommissionDecision.id,
        payload
      );
      if (!updated) {
        // Revert optimistic update on failure
        ...
        toast({
          title: 'Kushindwa',
          description: 'Imeshindwa kuweka maamuzi ya tume.',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  return (
    <div>
      <PageHeader
        title="Usimamizi wa Malalamiko"
        description="Wasilisha, tazama, na simamia malalamiko ya wafanyakazi."
      />
        ...
        <>
            ...
            <CardContent>
              {isLoading ? (
                ...
                employeeSubmittedComplaints.map((complaint) => (
                  <div
                    key={complaint.id}
                    className="mb-4 border p-4 rounded-md space-y-3 shadow-sm bg-background hover:shadow-md transition-shadow"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Aina: {complaint.complaintType} | Tarehe ya
                          Kuwasilisha:{' '}
                    ...
                    <p className="text-sm ">
                      ...
                      {complaint.details.substring(0, 150)}
                      {complaint.details.length > 150 ? '...' : ''}
                    </p>
                      ...
                      complaint.status.startsWith('Rejected by') && (
                        <Card className="mt-2 bg-red-50 border-red-200">
                          ...
                          <CardContent className="pb-2">
                            ...
                            {complaint.status.includes(
                              'Waiting submitter reaction'
                            ) && (
                              <div className="mt-3 pt-3 border-t border-red-200">
                                <p className="text-sm text-red-700 mb-2 font-medium">
                                  Unaweza kurekebisha na kuwasilisha upya
                                  lalamiko lako:
                                </p>
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() =>
                                    openResubmissionModal(complaint)
                                  }
                    ...
                    {complaint.status === 'Awaiting More Information' && (
                      <div className="mt-3 pt-3 border-t">
                        <p className="text-sm text-amber-700 font-medium mb-3">
                          Afisa mkaguzi ameomba maelezo zaidi. Tafadhali kagua
                          maoni yao na toa maelezo ya ziada hapa chini.
                        </p>
                        <Button
                          size="sm"
                          onClick={() => openProvideInfoModal(complaint)}
          ...
          <Card className="mb-6 shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Eleza lalamiko lako kwa uwazi. Unaweza kutumia zana ya AI
                ...
                >
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Fupisha lalamiko lako kwa ufupi"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Toa maelezo kamili ya lalamiko lako..."
                            {...field}
                            rows={6}
                          />
                        </FormControl>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Namba ya simu lazima iwe na tarakimu 10 zenye kuanza
                          na '0'
                        </p>
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="flex items-center">
                          ...
                          Namba ya Simu ya Mlezi / Mdhamini *
                        </FormLabel>
                        <FormControl>
                          <Input
                            type="tel"
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="flex items-center">
                          ...
                          Barua ya Lalamiko (PDF)
                        </FormLabel>
                        <FormControl>
                          <FileUpload
                            value={complaintLetterFile}
                            onChange={(objectKey) => {
                              const key = Array.isArray(objectKey)
                                ? objectKey[0]
                                : objectKey;
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          Pakia Vifungo vingine (Si Lazima, PDF)
                        </FormLabel>
                        <FormControl>
                          <FileUpload
                            value={evidenceFile}
                ...
                <Card className="mt-4 bg-secondary/50">
                  <CardHeader>
                    <CardTitle className="text-base">
                      Pendekezo la AI (Limesasishwa kwenye Fomu)
                    </CardTitle>
                  </CardHeader>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Malalamiko yanayohitaji hatua yako ya haraka - kagua, chukua
                    hatua, au omba maelezo zaidi.
                  </CardDescription>
                </div>
                <Button
                  variant="outline"
              ...
              {isLoading ? (
                ...
                activeComplaints.map((complaint) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <h3 className="font-semibold text-base">
                        {complaint.subject} (Type: {complaint.complaintType})
                      </h3>
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        ...
                        (complaint.status === 'Submitted' ||
                          ...
                          <>
                            ...
                            <Button
                              ...
                              variant="secondary"
                              onClick={() =>
                                openActionModal(complaint, 'request_info')
                              }
                            >
                              <Info className="mr-2 h-4 w-4" />
                              Omba Maelezo Zaidi
                            ...
                            <Button
                              ...
                              className="bg-green-600 hover:bg-green-700 text-white"
                              onClick={() =>
                                openActionModal(complaint, 'resolve')
                              }
                            >
                              <CheckCircle className="mr-2 h-4 w-4" />
                              Weka Kama Imetatuliwa (Moja kwa Moja)
                            </Button>
                            <Button
                              size="sm"
                              className="bg-blue-600 hover:bg-blue-700 text-white"
          ...
          {completedComplaints.length > 0 && (
            <Card className="shadow-lg mb-8">
              <CardHeader>
                ...
                <CardDescription>
                  Malalamiko yaliyopitia na kukamilika - historia yako ya kazi
                  iliyofanyika.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {completedComplaints.map((complaint) => (
                  <div
                    key={complaint.id}
                    className="mb-4 border p-4 rounded-md space-y-2 shadow-sm bg-secondary/20 hover:shadow-md transition-shadow"
                  >
                    <div className="flex justify-between items-start">
                      <h3 className="font-semibold text-base">
                        {complaint.subject} (Type: {complaint.complaintType})
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${
                          complaint.status === 'Mtumishi ameridhika na hatua'
                            ? 'bg-green-100 text-green-700'
                            : complaint.status === 'Closed - Satisfied'
                              ? 'bg-green-100 text-green-700'
                              : complaint.status.startsWith(
      ...
      {selectedComplaint && isDetailsModalOpen && (
        <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
          <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Maelezo ya Lalamiko: {selectedComplaint.id}
              </DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-4 py-4 text-sm max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                {format(parseISO(selectedComplaint.submissionDate), 'PPP p')}
              </div>
              <div>
                ...
                <Card className="mt-3 bg-blue-50 border-blue-200">
                  <CardHeader className="pb-2 pt-3">
                    <CardTitle className="text-base text-blue-700">
                      Maelezo ya Mawasiliano (Siri)
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="text-xs space-y-1 pb-3">
                    <p>
                      <strong>Simu ya Mlalamikaji:</strong>{' '}
                      {selectedComplaint.complainantPhoneNumber ||
                        'Haijatolewa'}
                    </p>
                    <p>
                      <strong>Simu ya Mlezi:</strong>{' '}
                      {selectedComplaint.nextOfKinPhoneNumber || 'Haijatolewa'}
                    </p>
                  </CardContent>
                </Card>
              )}
              ...
              {selectedComplaint.zanId && (
                <Card className="mt-3 bg-secondary/20">
                  ...
                  <CardContent className="text-xs space-y-1 pb-3">
                    <p>
                      <strong>Idara:</strong>{' '}
                      {selectedComplaint.department || 'Hakijulikani'}
                    </p>
                    <p>
                      <strong>Cheo/Nafasi:</strong>{' '}
                      {selectedComplaint.cadre || 'Hakijulikani'}
                    </p>
                  </CardContent>
                </Card>
              )}
                ...
                <div className="mt-2 space-y-2">
                  ...
                  selectedComplaint.attachments.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-sm">
                      Hakuna nyaraka zilizofungwa kwenye ombi hili.
                    </p>
                  )}
                </div>
              </div>
              ...
              {selectedComplaint.internalNotes && (
                <div className="mt-2 pt-2 border-t">
                  ...
                  <p className="mt-1 whitespace-pre-wrap bg-yellow-50 p-2 rounded border border-yellow-200">
                    {selectedComplaint.internalNotes}
                  </p>
                </div>
              )}
      ...
      {selectedComplaint &&
        isActionModalOpen &&
        (actionType === 'resolve' || actionType === 'request_info') && (
          <Dialog open={isActionModalOpen} onOpenChange={setIsActionModalOpen}>
            <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Kwa lalamiko la{' '}
                  <strong>{selectedComplaint.employeeName}</strong>.
                  {actionType === 'request_info'
                    ? ' Bainisha maelezo ya ziada yanayohitajika kutoka kwa mfanyakazi.'
                    : ' Toa maoni yako/sababu ya hatua hii.'}
                </DialogDescription>
              </DialogHeader>
              <div className="py-4 space-y-4">
                <div>
                  <Label htmlFor="officerActionCommentLegacy">
                    {actionType === 'request_info'
                      ? 'Maelezo/Ufafanuzi Unaohitajika kutoka kwa Mfanyakazi:'
                      : 'Maoni ya Afisa / Sababu ya Hatua:'}
                  </Label>
                  <Textarea
                ...
                <div>
                  <Label htmlFor="officerInternalNoteLegacy">
                    Maelezo ya Ndani (Si Lazima, kwa uwekaji kumbukumbu)
                  </Label>
                  <Textarea
      ...
      {currentRequestToAction && isRejectionModalOpen && (
        ...
        >
          <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Kataa Lalamiko: {currentRequestToAction.id}
              </DialogTitle>
              <DialogDescription>
                Tafadhali toa sababu ya kukataa lalamiko la{' '}
                <strong>{currentRequestToAction.employeeName}</strong>. Sababu
                hii itaonekana.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
      ...
      {selectedComplaintForInfo && isProvideInfoModalOpen && (
        <Dialog
          open={isProvideInfoModalOpen}
          onOpenChange={setIsProvideInfoModalOpen}
        >
          <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Kwa lalamiko:{' '}
                <strong>{selectedComplaintForInfo.subject}</strong>
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
      ...
      {isResubmissionModalOpen && selectedComplaintForResubmission && (
        <Dialog
          open={isResubmissionModalOpen}
          onOpenChange={setIsResubmissionModalOpen}
        >
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Rekebisha na Wasilisha Upya Lalamiko</DialogTitle>
              <DialogDescription>
                Rekebisha lalamiko lako kufuatia maoni ya afisa na uweke upya.
                Tafadhali hakikisha kurekebisha makosa yote yaliyooneshwa katika
            ...
            <div className="space-y-4">
                ...
                <Card className="bg-red-50 border-red-200">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-red-700">
                      Sababu ya Kukataliwa (Hapo awali):
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-red-600">
              ...
              <Form {...form}>
                <form
                  onSubmit={form.handleSubmit(handleResubmitComplaint)}
                  className="space-y-4"
                >
                  <FormField
                    control={form.control}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Andika kichwa kifupi cha lalamiko lako"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          Maelezo ya Lalamiko (Yaliyo Rekebiswa)
                        </FormLabel>
                        <FormControl>
                          <Textarea
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <Card className="bg-yellow-50 border-yellow-200">
                  <CardContent className="pt-4">
                    <p className="text-sm text-yellow-800">
                      <strong>Onyo:</strong> Maamuzi haya ni ya mwisho na hayana
                      kurudi nyuma. Lalamiko litafungwa kabisa.
                    </p>
                  </CardContent>
                </Card>
                ...
                <div>
                  ...
                  <Select
                    value={commissionDecisionType}
                    onValueChange={(value: 'resolved' | 'rejected') =>
                      setCommissionDecisionType(value)
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="resolved">
                        Lalamiko Limetatuliwa
                      </SelectItem>
                      <SelectItem value="rejected">
                        Lalamiko Limekataliwa
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
