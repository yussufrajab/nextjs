...
const mockRetirementFindUnique = vi.fn();
const mockRetirementUpdate = vi.fn();
...
vi.mock('@/lib/db', () => ({
  db: {
    ...
    retirementRequest: {
      findUnique: (...a: any[]) => mockRetirementFindUnique(...a),
      update: (...a: any[]) => mockRetirementUpdate(...a),
    },
...
vi.mock('@/lib/notifications', () => ({
  ...
  NotificationTemplates: {
    retirementHrrpApproved: () => ({ message: 'm', link: 'l' }),
    retirementHrrpRejected: () => ({ message: 'm', link: 'l' }),
    retirementPendingHrrpReview: () => ({ message: 'm', link: 'l' }),
  },
}));
...
function authedRequest(body: unknown): NextRequest {
  return new NextRequest('http://localhost:9002/api/retirement', {
    method: 'PATCH',
    headers: {
...
describe('PATCH /api/retirement — Commission workflow (parity with promotion)', () => {
  beforeEach(() => {
    mockValidateSession.mockReset();
    mockUserFindUnique.mockReset();
    mockRetirementFindUnique.mockReset();
    mockRetirementUpdate.mockReset();
    ...
    mockRetirementFindUnique.mockResolvedValue({
      id: 'req-1',
      employeeId: 'emp-1',
    ...
    mockRetirementUpdate.mockResolvedValue({
      id: 'req-1',
      employeeId: 'emp-1',
      submittedById: 'hro-1',
      ...
      Employee: { id: 'emp-1', name: 'Emp', zanId: 'Z1', institutionId: 'inst-1' },
      User_RetirementRequest_submittedByIdToUser: { id: 'hro-1', name: 'HRO', username: 'hro-user' },
      User_RetirementRequest_reviewedByIdToUser: null,
      User_RetirementRequest_hrrpReviewedByToUser: null,
    });
  });
  ...
  it('persists reviewStage="commission_review" when HHRMD forwards to the Commission', async () => {
    ...
    const updateArgs = mockRetirementUpdate.mock.calls[0][0];
  ...
  it('completes the workflow and retires the employee on Commission approval', async () => {
    mockRetirementUpdate.mockResolvedValue({
      id: 'req-1',
      employeeId: 'emp-1',
      submittedById: 'hro-1',
    ...
    // Commission approval must mark the employee as Retired.
