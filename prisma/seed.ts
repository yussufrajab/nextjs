...
async function main() {
  ...
  const institutionMap = new Map(
    seededInstitutions.map((inst) => [inst.name, inst.id])
  );

  console.log('Seeding employees...');
  for (const emp of EMPLOYEES) {
    const institutionId = institutionMap.get(
      emp.institution || 'TUME YA UTUMISHI SERIKALINI'
    );
    if (!institutionId) {
      console.warn(
        `Could not find institution for employee: ${emp.name}. Skipping.`
      );
      continue;
    }
  ...
  for (const user of USERS) {
    ...
    if (!institutionId) {
      console.warn(
        `Could not find institution ID for: ${institutionName}. Skipping user: ${user.name}`
      );
      continue;
    }
