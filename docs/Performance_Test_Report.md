...
const [
  ...
  db.confirmationRequest.findMany({ where: recentWhereClause, orderBy: { createdAt: 'desc' }, take: 5, include: { Employee: { select: { name: true } } } }),
  // ... 8 more parallel queries
]);
```
...
console.log(`Activities queries completed in ${Date.now() - activitiesStartTime}ms`);
...
2. **Progress Updates (Server-Sent Events):**
   ...
   const stream = new ReadableStream({
     start(controller) {
       sendProgress(controller, { percent: 0, message: 'Connecting to HRIMS...' });
       sendProgress(controller, { percent: 25, message: 'Fetching employee data...' });
       // ... etc
     }
   });
   ```
