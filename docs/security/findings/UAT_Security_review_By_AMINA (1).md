...
| 4.5 | Institution Filtering in APIs | 1. Call employee API<br>2. Call request API<br>3. Check responses | - API responses filtered<br>- No cross-institution data | ✅ All API routes | Verified in test 4.4. All 8 request-type endpoints (promotions, LWOP, confirmations, retirement, resignation, cadre-change, service-extension, termination) filter by institutionId for HRO/HRRP roles. CSC roles see all. | PASS | 68 uses of `shouldApplyInstitutionFilter()` across API routes |
