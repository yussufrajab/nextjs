...
import {
  ...
  Filter,
  Building,
  ListFilter as StatusFilterIcon,
  FileDown,
} from 'lucide-react';
    ...
    async (params: URLSearchParams = new URLSearchParams()) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/requests/track?${params.toString()}`
        );
        if (!response.ok) {
          throw new Error('Failed to fetch requests');
        }
        const result = await response.json();
        const data: TrackedRequest[] = Array.isArray(result)
          ? result
          : result.data || [];
        ...
        if (isTableView) {
          ...
          const statuses = Array.from(
            new Set(data.map((req) => req.status))
          ).sort();
          setRequestStatuses(statuses);
          const institutions = Array.from(
            new Set(
              data
                .map((req) => req.employeeInstitution)
          ...
          if (data.length === 0 && searchAttempted) {
            toast({
              ...
              description: `No requests found for ZanID: ${params.get('zanId')}.`,
            });
          } else if (searchAttempted) {
            toast({
              title: 'Requests Found',
              description: `Displaying requests for ZanID: ${params.get('zanId')}.`,
            });
          }
        }
  ...
  const handleExport = async (format: 'pdf' | 'excel') => {
    ...
    try {
      if (format === 'pdf') {
        ...
        doc.save(
          `${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_report.pdf`
        );
        toast({
          title: 'PDF Exported',
        ...
        const wsData = [
          ...
          ...filteredRequests.map((item) =>
            dataKeys.map((key) => (item as any)[key] ?? '')
          ),
        ];
        ...
        XLSX.writeFile(
          wb,
          `${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_report.xlsx`
        );
        toast({
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Monitor the status of requests submitted to the Civil Service Commission."
      />
      <>
            ...
            {isTableView ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
                <div className="space-y-1">
                  <Label htmlFor="fromDateFilter" className="flex items-center">
                ...
                <div className="space-y-1">
                  <Label htmlFor="toDateFilter" className="flex items-center">
                    <CalendarDays className="mr-2 h-4 w-4 text-primary" />
                    To Date
                  </Label>
                  <Input
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    id="zanIdFilter"
                    placeholder="Filter by ZanID"
                    value={zanIdFilter}
                    onChange={(e) => setZanIdFilter(e.target.value)}
                    disabled={isSearching}
                  />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <Label htmlFor="statusFilter" className="flex items-center">
                    <StatusFilterIcon className="mr-2 h-4 w-4 text-primary" />
                    Status
                  </Label>
                  <Select
                    value={statusFilter}
                ...
                <Button
                  ...
                  disabled={isSearching}
                  className="md:self-end lg:mt-0 mt-4"
                >
                  {isSearching ? (
            ...
            <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                <CardTitle>
                  {isTableView
                    ? 'Requests List'
                    : `Request Status for: ${selectedEmployee?.name} (${selectedEmployee?.zanId})`}
                </CardTitle>
                <CardDescription>
                  {isTableView &&
                    `Displaying ${filteredRequests.length} of the latest 100 requests matching your criteria.`}
                </CardDescription>
              </div>
              ...
              {filteredRequests.length > 0 ? (
                <>
                  <Table>
                    <TableCaption>
                      {isTableView
                        ? 'Overview of the 100 latest submitted requests.'
                        : `A list of recent requests for ${selectedEmployee?.name} (${selectedEmployee?.zanId}).`}
                    </TableCaption>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {paginatedRequests.map((request) => (
                        ...
                        >
                          ...
                          {isTableView && (
                            <TableCell>
                              {request.employeeInstitution || 'N/A'}
                            </TableCell>
                          )}
                ...
                <p className="text-muted-foreground text-center py-4">
                  {isTableView
                    ...
                    : `No requests found for ${selectedEmployee?.name} (${selectedEmployee?.zanId}).`}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg">
            <CardContent className="pt-6">
              <p className="text-muted-foreground text-center">
                Please enter an employee's ZanID above and click "Search
                Requests" to view their request history.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
          <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                <strong>Employee:</strong> {selectedRequestDetails.employeeName}{' '}
                (ZanID: {selectedRequestDetails.zanId}) <br />
                <strong>Institution:</strong>{' '}
                {selectedRequestDetails.employeeInstitution || 'N/A'} <br />
            ...
            <div className="space-y-4 py-4 text-sm max-h-[60vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                <div>
                  ...
                  <p>
                    {format(
                      parseISO(selectedRequestDetails.submissionDate),
                      'PPP p'
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p>
                    {format(
                      parseISO(selectedRequestDetails.lastUpdatedDate),
                      'PPP p'
                    )}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p>{selectedRequestDetails.currentStage}</p>
                </div>
