...
| 9.6 | Confidential information protection (complainant identity) | ✅ Implemented | `src/lib/complaint-confidentiality.ts:74-129` (`canSeeComplainantIdentity`); `src/app/api/complaints/route.ts:228-269` (GET redaction + flags), `:33-72` (POST `confidential` + `resolveConfidential`), `:80-114` (redacted notification/email name); `src/app/api/complaints/[id]/route.ts:254-272` (PUT `confidential` enforcement), `:360-392` (PUT response redaction); `prisma/schema.prisma` (`Complaint.confidential`); `prisma/migrations/20260725020000_add_complaint_confidential/migration.sql` | **Fixed (was ❌).** New `confidential Boolean @default(false)` column on `Complaint` (migration applied). A shared need-to-know helper `canSeeComplainantIdentity(role, userId, { complainantId, assignedOfficerRole, confidential })` gates the complainant's identity PII (`employeeId`, `employeeName`, `zanId`, `complainantPhoneNumber`, `nextOfKinPhoneNumber`): the complainant sees their own data; the **exactly-assigned** handling officer (DO/HHRMD) is the owning handler and sees identity — the co-reviewer tier (a DO viewing an HHRMD-assigned complaint, or vice-versa) is **redacted** (they may act on the workflow per Req 9.7 but do not need the identity); CSCS (commission/escalation) and Admin (override) see identity for **non-confidential** complaints; **confidential** complaints tighten access to complainant + exactly-assigned officer only — even CSCS/Admin are redacted (whistleblower protection: the adjudicating body decides on the case merits without the complainant's identity). Redaction is applied in the GET list (`redactComplainantIdentity` clears PII + sets `complainantIdentityRedacted:true`) and the PUT `[id]` response (the response is spread into the caller's cached row); both responses now expose a `confidential` flag. Harassment ("Unyanyasaji") complaints are always confidential (`resolveConfidential` — a submitter/officer cannot downgrade one); other types are confidential on request. Confidential complaints never broadcast the complainant's real name in officer notifications/emails (redacted label used). Officers may set the `confidential` flag on PUT (employees cannot — excluded from `EMPLOYEE_ALLOWED_FIELDS`). Frontend (`complaints/page.tsx`) shows a confidentiality banner on confidential complaints. Covered by `src/lib/complaint-confidentiality.test.ts` (16 cases). |
