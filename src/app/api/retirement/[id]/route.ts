...
async function handleUpdate(
  ...
  { params }: { params: Promise<{ id: string }> }
    ...
    const existingRequest = await db.retirementRequest.findUnique({
      where: { id },
      include: { Employee: { select: { id: true, institutionId: true } } },
    });

    if (!existingRequest) {
      return new NextResponse('Retirement request not found', { status: 404 });
    }
...
export const PUT = wrapHandler(handleUpdate, 'retirement');
export const PATCH = wrapHandler(handleUpdate, 'retirement');
...
export const DELETE = wrapHandler(async (
  ...
  await db.retirementRequest.delete({ where: { id } });
