...
export default function RetirementPage() {
  ...
  const [illnessLeaveLetterFile, setIllnessLeaveLetterFile] =
    useState<string>('');
  const [letterOfRequestFile, setLetterOfRequestFile] = useState<string>('');
  const [minRetirementDate, setMinRetirementDate] = useState('');
  const [ageEligibilityError, setAgeEligibilityError] = useState<string | null>(
    null
  );
  ...
  const [correctedIllnessDescription, setCorrectedIllnessDescription] =
    useState('');
  const [correctedDelayReason, setCorrectedDelayReason] = useState('');
  const [correctedMedicalFormFile, setCorrectedMedicalFormFile] =
    useState<string>('');
  const [correctedIllnessLeaveLetterFile, setCorrectedIllnessLeaveLetterFile] =
    useState<string>('');
  const [correctedLetterOfRequestFile, setCorrectedLetterOfRequestFile] =
    useState<string>('');
  const [correctedDelayDocumentFile, setCorrectedDelayDocumentFile] =
    useState<string>('');
  const [correctedAgeEligibilityError, setCorrectedAgeEligibilityError] =
    useState<string | null>(null);
  const [showCorrectedDelayFields, setShowCorrectedDelayFields] =
    useState(false);
  ...
  const fetchRequests = useCallback(
    async (isRefresh = false, page = currentPage) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error('Failed to fetch retirement requests');
        ...
        if (isRefresh) {
          toast({
            title: 'Refreshed',
            description: 'Retirement requests have been updated.',
            duration: 2000,
          });
        }
      } catch (error) {
        toast({
          title: 'Error',
          description: 'Could not load retirement requests.',
          variant: 'destructive',
        });
      } finally {
        if (isRefresh) {
  ...
  useEffect(() => {
    if (
      ...
      const formattedMonth = String(retirementDate.getMonth() + 1).padStart(
        2,
        '0'
      );
  ...
  useEffect(() => {
    ...
    if (
      ...
      const [birthYear, birthMonth, birthDay] = birthDateStr
        .split('-')
        .map(Number);
      const [retireYear, retireMonth, retireDay] = retirementDateStr
        .split('-')
        .map(Number);
      ...
      if (
        ...
        (retireMonth === birthMonth && retireDay < birthDay)
      ) {
      ...
      if (retirementType === 'compulsory') {
        if (ageAtRetirement > COMPULSORY_RETIREMENT_AGE) {
          ...
          setAgeEligibilityError(
            `Employee will be ${ageAtRetirement} and not meet the compulsory retirement age (${COMPULSORY_RETIREMENT_AGE}) by the proposed date.`
          );
        }
      } else if (retirementType === 'voluntary') {
        if (ageAtRetirement >= COMPULSORY_RETIREMENT_AGE) {
          setAgeEligibilityError(
            `Employee is aged ${ageAtRetirement} and qualifies for compulsory retirement. Please select 'Compulsory (Age 60)' as the retirement type.`
          );
        } else if (ageAtRetirement < VOLUNTARY_RETIREMENT_AGE) {
          setAgeEligibilityError(
            `Employee will be ${ageAtRetirement} and not meet the voluntary retirement age (${VOLUNTARY_RETIREMENT_AGE}) by the proposed date.`
          );
        }
      }
    }
    ...
    if (requestToCorrect && correctedRetirementType && correctedRetirementDate) {
      ...
      if (correctedRetirementType === 'compulsory') {
        if (ageAtRetirement > COMPULSORY_RETIREMENT_AGE) {
          ...
          setCorrectedAgeEligibilityError(
            `Employee will be ${ageAtRetirement} and not meet the compulsory retirement age (${COMPULSORY_RETIREMENT_AGE}) by the proposed date.`
          );
        }
  ...
  const handleEmployeeFound = (employee: Employee) => {
    ...
    if (hasPending) {
      ...
      toast({
        ...
        description:
          'A retirement request for this employee is already being reviewed. You cannot submit another request until the current one is completed.',
        variant: 'destructive',
        duration: 6000,
      });
    }
    ...
    if (!employee.dateOfBirth) {
      toast({
        ...
        description:
          'Employee date of birth is missing. Age validation cannot be performed.',
        variant: 'default',
        duration: 5000,
      });
    }
  ...
  const handleSubmitRetirementRequest = async () => {
    ...
    if (!retirementType) {
      toast({
        title: 'Submission Error',
        description: 'Please select a retirement type.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!retirementDate && retirementType !== 'illness') {
      toast({
        title: 'Submission Error',
        description: 'Please select a retirement date.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!letterOfRequestFile) {
      toast({
        title: 'Submission Error',
        description: 'Please upload the letter of request.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (retirementType === 'illness') {
      if (!medicalFormFile) {
        toast({
          title: 'Submission Error',
          description: 'Please upload the medical form for illness retirement.',
          variant: 'destructive',
        });
        return;
      }
      if (!illnessDescription) {
        toast({
          title: 'Submission Error',
          description: 'Please describe the illness.',
          variant: 'destructive',
        });
        return;
      }
    ...
    if (retirementType === 'illness') {
      ...
      if (illnessLeaveLetterFile)
        documentObjectKeys.push(illnessLeaveLetterFile);
    }
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : 'Could not submit the retirement request.';
      toast({
  ...
  const handleCommissionDecisionSubmit = async () => {
    ...
    try {
      ...
      const actionDescription = commissionDecisionType === 'approved'
        ? `Retirement approved by Commission.`
        : 'Retirement request rejected by Commission';
  ...
  const handleConfirmResubmit = async (request: RetirementRequest | null) => {
    ...
    if (
      ...
      (correctedRetirementType !== 'illness' && !correctedRetirementDate) ||
      !correctedLetterOfRequestFile
    ) {
      toast({
        title: 'Submission Error',
        description: 'All required fields and documents must be provided.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (correctedAgeEligibilityError) {
      toast({
        ...
        description: correctedAgeEligibilityError,
        variant: 'destructive',
      });
      return;
    }
    ...
    if (correctedRetirementType === 'illness' && !correctedMedicalFormFile) {
      toast({
        title: 'Submission Error',
        description: 'Medical form is required for illness retirement.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (correctedRetirementType === 'illness') {
        if (correctedMedicalFormFile)
          correctedDocumentObjectKeys.push(correctedMedicalFormFile);
        if (correctedIllnessLeaveLetterFile)
          correctedDocumentObjectKeys.push(correctedIllnessLeaveLetterFile);
      }

      if (correctedDelayDocumentFile)
        correctedDocumentObjectKeys.push(correctedDelayDocumentFile);

      // Convert date string to ISO-8601 DateTime format (null for illness retirement)
      const proposedDateTime = correctedRetirementDate
        ? new Date(correctedRetirementDate).toISOString()
        : null;
      ...
      const success = await handleUpdateRequest(
        request.id,
        payload,
        `Retirement request resubmitted for review`
      );
        ...
        <Card className="mb-6 shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              ...
              retirement details and upload required PDF documents.
            </CardDescription>
          </CardHeader>
              ...
              <div className="space-y-6 pt-2">
                  ...
                  <div className="p-4 rounded-md border bg-secondary/20 space-y-3 shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3 text-sm">
                      ...
                      <div className="md:col-span-2 lg:col-span-3">
                        ...
                        <p
                          className={`font-semibold ${cannotSubmitRetirement ? 'text-destructive' : 'text-green-600'}`}
                        >
                ...
                {hasPendingRetirement && (
                  <Alert variant="destructive" className="mt-4">
                    ...
                    <AlertDescription>
                      A retirement request for this employee is already being
                ...
                <div
                  className={`space-y-4 ${(ageEligibilityError && !showDelayFields) || cannotSubmitRetirement || hasPendingRetirement ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <h3 className="text-lg font-medium text-foreground">
                    Retirement Details &amp; Documents (PDF Only)
                  </h3>
                  <div>
                    <Label
                      htmlFor="retirementType"
                    ...
                    <Select
                      ...
                      onValueChange={setRetirementType}
                      disabled={
                        isSubmitting ||
                        (ageEligibilityError && !showDelayFields) ||
                        cannotSubmitRetirement ||
                  ...
                  {retirementType === 'illness' && (
                    <>
                      ...
                      <div>
                        <Label
                          htmlFor="medicalFormFile"
                          className="flex items-center"
                        >
                          <Stethoscope className="mr-2 h-4 w-4 text-primary" />
                          Upload Medical Form (Required, PDF Only)
                        </Label>
                        <FileUpload
                          folder="retirement"
                      ...
                      <div>
                        <Label
                          htmlFor="illnessLeaveLetterFile"
                          className="flex items-center"
                        >
                          <ClipboardCheck className="mr-2 h-4 w-4 text-primary" />
                          Upload Leave Due to Illness Letter (Required, PDF
                          Only)
                        </Label>
                        <FileUpload
                          folder="retirement"
                  ...
                  {showDelayFields && (
                    <div className="space-y-4 pt-2 border-t border-yellow-300">
                      ...
                      >
                        ...
                        <AlertDescription>
                          This employee is over {COMPULSORY_RETIREMENT_AGE}.
                          Please provide a reason for the delay and upload a
                          supporting document.
                        </AlertDescription>
                      </Alert>
                      <div>
                      ...
                      <div>
                        <Label
                          htmlFor="delayDocumentFile"
                          className="flex items-center"
                        >
                          <FileText className="mr-2 h-4 w-4 text-primary" />
                          Upload Supporting Document for Delay (e.g., Extension
                          Letter) (Required, PDF Only)
                        </Label>
                        <FileUpload
                          folder="retirement"
                  ...
                  <div>
                    <Label
                      htmlFor="letterOfRequestRetirement"
                      className="flex items-center"
                    >
                      <FileText className="mr-2 h-4 w-4 text-primary" />
                  ...
                  {retirementType !== 'illness' && (
                    <p className="text-xs text-muted-foreground">
                      Note: Proposed retirement date must be at least 6 months
                      from today. Age validation is based on the proposed
                      retirement date.
                    </p>
                  )}
                </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Track the status of retirement requests you have submitted.
                </CardDescription>
              </div>
              <Button
                variant="outline"
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Review, approve, or reject pending retirement requests.
                </CardDescription>
              </div>
              <Button
                variant="outline"
            ...
            <div className="space-y-4 py-4 text-sm max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-1">
                ...
                {selectedRequest.delayReason && (
                  <div className="grid grid-cols-3 items-start gap-x-4 gap-y-2">
                    <Label className="text-right font-semibold text-yellow-700 pt-1">
                      Reason for Delay:
                    </Label>
      ...
      {currentRequestToAction &&
        ...
        >
          <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Reject Retirement Request: {currentRequestToAction.id}
              </DialogTitle>
              <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <FileUpload
                ...
                accept=".pdf"
                maxSize={1}
                folder="retirement/commission-letters"
          ...
          <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="py-4 space-y-4">
              <Alert variant="default">
                ...
                <AlertDescription>
                  Update the retirement details and re-attach all required PDF
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="correctedRetirementType">
                    Retirement Type
                  </Label>
                  <Select
                    value={correctedRetirementType}
                    onValueChange={setCorrectedRetirementType}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select retirement type" />
                    </SelectTrigger>
                ...
                {correctedRetirementType !== 'illness' && (
                  <div>
                    <Label htmlFor="correctedRetirementDate">
                      Proposed Retirement Date
                    </Label>
                    <Input
                      ...
                      onChange={(e) =>
                        setCorrectedRetirementDate(e.target.value)
                      }
                      min={minRetirementDate}
                    />
                  </div>
                ...
                {correctedRetirementType === 'illness' && (
                  <div>
                    <Label htmlFor="correctedIllnessDescription">
                      Illness Description
                    </Label>
                    <Textarea
                      ...
                      onChange={(e) =>
                        setCorrectedIllnessDescription(e.target.value)
                      }
                    />
                  </div>
                )}
                ...
                {showCorrectedDelayFields && (
                  <div>
                    <Label htmlFor="correctedDelayReason">
                      Delay Reason (Required for retirement beyond age 60)
                    </Label>
                    <Textarea
              ...
              <div className="space-y-4">
                ...
                {correctedRetirementType === 'illness' && (
                  <>
                    ...
                    <div>
                      <Label className="flex items-center mb-2">
                        <FileText className="mr-2 h-4 w-4 text-primary" />
                        Upload Illness Leave Letter (Optional)
                      </Label>
                      <FileUpload
                        folder="retirement"
                ...
                {showCorrectedDelayFields && (
                  <div>
                    <Label className="flex items-center mb-2">
                      <ClipboardCheck className="mr-2 h-4 w-4 text-primary" />
                      Upload Delay Document (Optional)
                    </Label>
                    <FileUpload
                      folder="retirement"
