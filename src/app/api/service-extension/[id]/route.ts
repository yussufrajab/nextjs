...
async function handleUpdate(
  ...
  { params }: { params: Promise<{ id: string }> }
    ...
    const existingRequest = await db.serviceExtensionRequest.findUnique({
      where: { id },
      include: { Employee: { select: { id: true, institutionId: true } } },
    });

    if (!existingRequest) {
      return new NextResponse('Service extension request not found', { status: 404 });
    }
