...
> **Re-audit note (2026-08-04).** Re-verified against the current working tree. A new IP-ban-on-abuse feature landed (11 commits, `IpBan` model + migration `20260804000000_add_ip_ban`) — an additive brute-force mitigation layer that strengthens Req 1.8 (rate limiting) and Req 26.1 (failed-login monitoring) but does not change any ✅/⚠️/❌ verdict or the 130/53/23 totals. All 23 ❌ and 53 ⚠️ gap rows were re-checked against the current code and remain accurate; none of the 11 recent commits touched any gap area. See the updated evidence on rows 1.8 and 26.1, and the new "Closed in this pass (2026-08-04)" note at the end.
...
|----:|----------------------|:------:|:------:|:------:|:------:|
| 1 | Authentication & Identity Assurance | 13 | 12 | 1 | 0 |
...
| 6 | Employee Creation Integrity | 8 | 6 | 2 | 0 |
| 7 | Bulk Upload Security | 9 | 6 | 3 | 0 |
| 8 | Workflow Security & Approval Integrity | 9 | 8 | 1 | 0 |
| 9 | Complaint Management Security | 7 | 7 | 0 | 0 |
| 10 | File & Document Security | 8 | 5 | 2 | 1 |
| 11 | HRIMS Integration Security | 8 | 5 | 2 | 1 |
| 12 | Reporting & Export Security | 8 | 5 | 1 | 2 |
| 13 | Notification Security | 6 | 3 | 3 | 0 |
| 14 | Administrative Security | 8 | 6 | 2 | 0 |
...
| 26 | Security Monitoring & Detection | 6 | 5 | 0 | 1 |
...
**Headline result (2026-07-21 re-audit; updated 2026-08-04):** 132/206 controls fully implemented (64.1%), 51/206 partial (24.8%), 23/206 not implemented (11.2%) — up from 94/79/33 at the 2026-07-18 baseline. The strongest areas are Session Security (Req 2), Institution Isolation (Req 4), Authorization (Req 3), Workflow Security (Req 8), and Non-Repudiation (Req 19). The weakest areas remain Data Classification (Req 22), Audit Log Protection (Req 21), Synchronization Accountability (Req 29), and Administrative Change Control (Req 28).
...
|---|---|---|---|---|
| 1.1 | Multi-Factor Authentication (designated roles) | ✅ Implemented | `src/app/api/auth/login/route.ts:421-448`; `src/lib/constants.ts:14` (`MFA_REQUIRED_ROLES`); `src/lib/mfa-utils.ts:23-57`; `prisma/schema.prisma:412-471` | **Fixed (was ⚠️).** Email-OTP MFA is now required for privileged roles (Admin, CSCS, HHRMD) via `MFA_REQUIRED_ROLES` in `src/lib/constants.ts`, regardless of email presence. A privileged role with no email is **blocked** (403 `MFA_REQUIRED_NO_EMAIL`) instead of silently skipped — the login cannot proceed without an MFA channel. Non-privileged roles with an email still get MFA (existing behavior); non-privileged roles without email complete login directly. The `requireMfa` per-user flag is available as a future extension point (not yet added to the schema). Tests in `src/app/api/auth/login/mfa-role.test.ts` (6 cases: Admin/CSCS/HHRMD no-email→403, Admin with email→MFA_REQUIRED, HRO no-email→login, HRO with email→MFA). |
...
| 1.8 | Login rate limiting per IP/user | ✅ Implemented | `src/lib/rate-limiter.ts:198-204` (per-IP fixed window via `withRateLimit`); `src/lib/rate-limiter.ts:checkRateLimitSliding` + `buildUserRateLimitKey` (per-user sliding window); `src/app/api/auth/login/route.ts:46-75` (per-IP ban gate); `src/lib/ip-ban-utils.ts` (IP ban); `prisma/schema.prisma:552-569` (`IpBan` model); `src/app/api/admin/ban-ip/route.ts`; `src/app/api/admin/unban-ip/route.ts`; `src/app/api/admin/ip-bans/route.ts`; `src/app/api/auth/ip-ban-status/route.ts` | **Fixed (was ⚠️).** Per-IP fixed window (5/60s, fail-closed) retained; added a per-user sliding-window counter keyed `ratelimit:user:${normalizedUsername}:auth` (Redis ZSET + atomic Lua) checked before the DB lookup so distributed brute-force against one account (and guesses against non-existent usernames) is throttled regardless of source IP. Username normalized (trim+lowercase, capped 256) so case variants share a bucket. Reuses the `auth` tier config (env-overridable via `RATE_LIMIT_AUTH_LIMIT` for E2E) and fails closed (503) when Redis is down. **Enhanced (2026-08-04):** an IP-ban-on-abuse layer now sits ahead of the DB lookup (`login/route.ts:46-75`): `autoUnbanExpiredIps()` clears expired standard bans, then `isIpBanned()` (reads Postgres `IpBan`, survives a Redis outage) blocks the request with 403 `IP_BLOCKED` + `retryAfter`. Two auto-ban triggers — `recordFailedLoginFromIp` (10 failed logins within `IP_BAN_WINDOW_SECONDS`=900s) and `recordRateLimitHit` (3 auth-tier 429s, wired in `rate-limiter.ts:371`) — feed `banIp()`, which escalates on a ladder (30 min → 2h → 24h → security-only). Admin can manually ban/unban with step-up reauth (`admin/ban-ip`, `admin/unban-ip`); `admin/ip-bans` lists bans; `auth/ip-ban-status` is the public status read. Trusted IPs (`IP_TRUSTED_IPS`) are exempt. Env-overridable thresholds for E2E. Audit events: `IP_BANNED`/`IP_BANNED_UPGRADED`/`IP_AUTO_UNBANNED`/`ADMIN_IP_BAN`/`ADMIN_IP_UNBAN`. Tests in `src/lib/ip-ban-utils.test.ts`, `src/lib/rate-limiter.ip-ban.test.ts`, `src/app/api/auth/login/ip-ban.test.ts`, `src/app/api/admin/ban-ip.route.test.ts`, `src/app/api/admin/ip-bans.route.test.ts`. |
...
| 6.5 | Duplicate detection (fuzzy name/DOB/institution) | ✅ Implemented | `src/lib/employee-duplicate-detection.ts` (`normalizeName`, `levenshtein`, `nameSimilarity`, `dobDayBounds`, `findFuzzyDuplicate`, `isWithinFileFuzzyDuplicate`); `manual-entry/route.ts` (fuzzy block after the exact-key checks); `bulk-upload/route.ts` (within-file fuzzy pass + per-row DB fuzzy check) | **Fixed (was ❌).** Added a shared, zero-dependency fuzzy duplicate detector that complements the exact-key uniqueness checks (6.2–6.4). The DB query is scoped to `institutionId` + the `dateOfBirth` calendar day (`dobDayBounds` → a UTC `[start-of-day, start-of-next-day)` range, robust to any stored time component), then normalized Levenshtein name similarity is compared against `DUPLICATE_NAME_SIMILARITY_THRESHOLD = 0.85`. `normalizeName` lowercases, replaces punctuation with spaces, and collapses whitespace so `"O'Brien"≈"OBrien"`, `"Jane D. Doe"≈"jane d doe"`, and single-transliteration-variant names ("Mohammed" vs "Mohammad", "Jane" vs "Jayne") score ≥ threshold while unrelated names of similar length do not. **Manual-entry** blocks creation with 409 (consistent with the exact-key checks) and surfaces the existing record's zanId/name + match % so the HRO can verify before re-attempting. **Bulk-upload** adds a within-file fuzzy pass (`isWithinFileFuzzyDuplicate` — same DOB + similar name across rows, since `institutionId` is session-forced) AND a per-row DB fuzzy check, both reported as per-row errors. The check no-ops gracefully (no duplicate) when `institutionId` is missing or `dateOfBirth` does not parse, so the exact-key checks still run. Covered by `src/lib/employee-duplicate-detection.test.ts` (26 cases), `src/app/api/employees/manual-entry/route.test.ts` (409 on near-match, 201 on unrelated name, DB-query scoping), and `src/app/api/employees/bulk-upload/route.test.ts` (within-file + DB fuzzy flagging, clean-file pass, different-DOB no-flag). |
...
| 7.7 | Import audit logging (batch ID, counts) | ✅ Implemented | `bulk-upload/route.ts` (POST validate + PUT confirm-and-create); `bulk-upload.tsx` (client echoes batchId) | **Fixed (was ⚠️).** POST (validate) generates one `const batchId = uuidv4()` at the start of the handler, includes it in the `UPLOADED` audit event `additionalData` (alongside totalRows/validRows/invalidRows), and surfaces it in the validation response `data.batchId`. The client (`bulk-upload.tsx`) echoes `validationResult.batchId` back in the PUT request body; the PUT confirm-and-create handler reads `body.batchId` (falling back to a fresh `uuidv4()` only if the client omitted it) and stamps it into every per-row `CREATED` audit event `additionalData` next to `batchRow`. One import now emits one `UPLOADED` + N `CREATED` events all sharing the same `batchId`, linking the import end-to-end. Covered by `bulk-upload/route.test.ts` (shared batchId across CREATED events; fallback when client omits batchId). |
...
| 9.2 | Complaint access control (involved parties) | ✅ Implemented | `src/app/api/complaints/[id]/route.ts` (new `GET`); `src/lib/complaint-privacy.ts`; `src/lib/workflow-access.ts` (`denyWorkflowAccess`) | **Fixed (was ⚠️).** Added `GET /api/complaints/[id]` enforcing involved-party visibility: only the complainant or a complaint-handling officer tier (DO/HHRMD/CSCS/Admin) may read a single complaint; everyone else (HRMO, HRO, HRRP, PO, non-owning EMPLOYEE) is denied with a 403 written to the audit trail via the existing `denyWorkflowAccess` path (`blockReason: NOT_INVOLVED_PARTY`, `requestMethod: GET`). Complainant identity PII is masked via the new `redactComplainantPii` before return. Covered by `src/app/api/complaints/[id]/route.test.ts` (404, complainant/assigned-officer full, co-reviewer masked, non-involved 403 audited, Admin full/masked-by-confidential). |
...
| 9.6 | Confidential information protection (complainant identity) | ✅ Implemented | `src/lib/complaint-confidentiality.ts:74-129` (`canSeeComplainantIdentity`); `src/lib/complaint-privacy.ts` (`redactComplainantPii` masking); `src/app/api/complaints/route.ts` (GET list redaction + flags), `:33-72` (POST `confidential` + `resolveConfidential`), `:80-114` (redacted notification/email name); `src/app/api/complaints/[id]/route.ts:254-272` (PUT `confidential` enforcement), `:360-392` (PUT response redaction), new `GET` (masked read); `prisma/schema.prisma` (`Complaint.confidential`); `prisma/migrations/20260725020000_add_complaint_confidential/migration.sql` | **Fixed (was ❌), then tightened.** New `confidential Boolean @default(false)` column on `Complaint` (migration applied). A shared need-to-know helper `canSeeComplainantIdentity(role, userId, { complainantId, assignedOfficerRole, confidential })` gates the complainant's identity PII (`employeeId`, `employeeName`, `zanId`, `complainantPhoneNumber`, `nextOfKinPhoneNumber`): the complainant sees their own data; the **exactly-assigned** handling officer (DO/HHRMD) is the owning handler and sees identity — the co-reviewer tier (a DO viewing an HHRMD-assigned complaint, or vice-versa) is **redacted** (they may act on the workflow per Req 9.7 but do not need the identity); CSCS (commission/escalation) and Admin (override) see identity for **non-confidential** complaints; **confidential** complaints tighten access to complainant + exactly-assigned officer only — even CSCS/Admin are redacted (whistleblower protection). New `src/lib/complaint-privacy.ts` `redactComplainantPii(complaint, { viewerRole, viewerUserId, assignedOfficerId })` now masks (rather than fully nulls) the read paths: a non-owning viewer sees `zanId`/phones masked to `***1234`, `employeeId` dropped, `employeeName` rendered as initials, and `complainantIdentityRedacted:true`; the complainant, the specifically-assigned officer (`assignedOfficerId` user-id match, e.g. `reviewedById`), and (for non-confidential complaints) Admin/CSCS see the full identity. `complainantId` is consumed for the decision and **stripped from the output** so the complainant's user id is never leaked. Applied in the GET list and the new `GET /api/complaints/[id]`; the PUT `[id]` response keeps its existing nested redaction. Harassment ("Unyanyasaji") complaints are always confidential (`resolveConfidential`); other types are confidential on request and never broadcast the complainant's real name in officer notifications/emails. Officers may set the `confidential` flag on PUT (employees cannot). Frontend (`complaints/page.tsx`) shows a confidentiality banner. Covered by `src/lib/complaint-confidentiality.test.ts` (16 cases), `src/lib/complaint-privacy.test.ts` (masking + visibility branches), and `src/app/api/complaints/[id]/route.test.ts` (GET visibility + masking). |
...
| 11.2 | Trusted source validation (whitelisted endpoint, IP/cert, API key) | ✅ Implemented | `src/lib/hrims-config.ts` (allow-list + TLS/https + `HrimsConfigError`); `hrims/sync-employee/route.ts`, `hrims/sync-documents/route.ts`, `hrims/sync-certificates/route.ts`, `hrims/fetch-employee/route.ts` | **Fixed (was ❌).** Removed caller-supplied `hrimsApiUrl`/`hrimsApiKey` from the three Zod schemas + handlers (and the background-sync payloads); all four HRIMS routes now source the endpoint + credentials exclusively from `getHrimsApiConfig()` (`fetch-employee`'s safe pattern). `src/lib/hrims-config.ts` now enforces a trusted-source policy at config-load: `HRIMS_ALLOWED_HOSTS` (comma-separated, case-insensitive) gates the resolved host — DB-stored or env default — and a violation throws a typed `HrimsConfigError` (`HRIMS_HOST_NOT_ALLOWED`). Production requires `https` (`HRIMS_API_SCHEME=https`) unless the operator explicitly opts in to insecure HTTP (`HRIMS_ALLOW_INSECURE_HTTP=true`); TLS certificate validation is left at undici's secure default and is never disabled (no `rejectUnauthorized:false`). Routes catch `HrimsConfigError` and return **400 + a `HRIMS_SYNC_FAILED` audit event** (`additionalData.reason = <code>`) so a misconfigured or tampered endpoint can never proxy the server at an arbitrary host (SSRF) or exfiltrate the API key. Env documented in `.env.example`/`.env.production`. Covered by `src/lib/hrims-config.test.ts` (allow-list incl. DB-stored host, https-in-production, opt-in, scheme fallback) and `src/app/api/hrims/fetch-employee/route.test.ts` (non-allowlisted host → 400 + `HRIMS_SYNC_FAILED`; body `hrimsApiUrl`/`hrimsApiKey` ignored). |
...
| 14.2 | Privileged access control (MFA for admin console) | ✅ Implemented | `src/lib/reauth.ts`; `auth/reauth/route.ts:128-129`; `admin/hrims-settings/route.ts:45-49` (`requireReauth`); `auth/login/route.ts:421-448`; `src/lib/constants.ts:14` (`MFA_REQUIRED_ROLES`) | **Fixed (was ⚠️).** Step-up reauth (with OTP if MFA enabled) on sensitive sub-actions. Admin MFA is now required via `MFA_REQUIRED_ROLES` (Req 1.1 fix). `admin/hrims-settings` PUT now enforces `requireReauth(request, 'admin.hrims-settings', auth)` before processing — closing the last reauth gap. |
...
| # | Control | Status | Evidence (file:line) | Remarks / Gap |
|---|---|---|---|---|
| 26.1 | Failed login monitoring | ✅ Implemented | `login/route.ts:63-315`; `account-lockout-utils.ts`; `suspicious-login-detector.ts:26`; `audit-logger.ts:270`; `src/lib/ip-ban-utils.ts:230-251,272-336`; `src/lib/audit-logger.ts:69-73` (`IP_BANNED`/`IP_BANNED_UPGRADED`/`IP_AUTO_UNBANNED`/`ADMIN_IP_BAN`/`ADMIN_IP_UNBAN`) | `LOGIN_FAILED` + lockout tracking + suspicious-success detection. **Enhanced (2026-08-04):** IP-ban-on-abuse now auto-bans source IPs on sustained abuse — `recordFailedLoginFromIp` (10 failed logins in `IP_BAN_WINDOW_SECONDS`=900s) and `recordRateLimitHit` (3 auth-tier 429s, fed from `rate-limiter.ts:371`) both feed `banIp()`, which escalates on a ladder (30 min → 2h → 24h → security-only); `autoUnbanExpiredIps()` clears expired standard bans on each login attempt. All ban transitions emit tamper-evident audit events (`IP_BANNED` etc.) via `logAuditEvent`. Env-overridable thresholds; trusted IPs exempt. Admin manual ban/unban with step-up reauth (`admin/ban-ip`, `admin/unban-ip`). |
| 26.2 | Privilege escalation detection | ✅ Implemented | `src/app/api/users/[id]/route.ts` (PUT role-change); `src/lib/role-privilege.ts` (ranking + burst) | **Fixed (was ⚠️).** The PUT role-change handler now classifies every role change with `src/lib/role-privilege.ts`: a static `PRIVILEGE_ROLE_RANK` orders the CSMS tiers (EMPLOYEE<HRO<HRRP/PO<DO/HRMO<HHRMD<CSCS<Admin) so `isPrivilegeEscalation(prev,new)` flags strict rank increases (lateral moves and demotions are NOT escalations). The `USER_UPDATED` audit row carries a `privilegeEscalation` flag so escalations are discoverable from the audit trail. On an escalation the handler emits a CRITICAL `POTENTIAL_BREACH` via `logAuditEvent` — which is already wired to the outbound `dispatchSecurityAlert` pipeline (webhook/SIEM + email, Req 26.6). Two triggers: (1) the new role is a high-privilege tier (Admin/HHRMD/CSCS) — a single such escalation pages the SOC immediately; (2) `detectEscalationBurst` counts prior `USER_UPDATED` rows carrying `privilegeEscalation=true` in a look-back window (env `PRIVILEGE_ESCALATION_BURST_WINDOW_SECONDS`, default 1h; threshold `PRIVILEGE_ESCALATION_BURST_THRESHOLD`, default 3) and flags an unusual run, catching distributed/slow accrual that no single change would flag. Burst lookup is wrapped so an audit-DB outage never breaks the role-change flow. Lateral/non-escalating changes emit no alert. Covered by `src/lib/role-privilege.test.ts` (ranking, escalation classification, burst counting, env config) and `src/app/api/users/[id]/route.test.ts` (Admin/HHRMD/CSCS escalation → CRITICAL; lateral/demotion/institution-only → no alert; burst → CRITICAL; audit-query-failure resilience). |
...
8. ~~**Complainant confidentiality** (Req 9.6): redact/anonymize complainant identity for non-resolving officers; add a `GET /api/complaints/[id]` with involved-party visibility (Req 9.2). (Resolution authorization Req 9.7 is already fixed.)~~ **Closed (2026-07-25)** via `src/lib/complaint-confidentiality.ts` + wiring in `complaints/route.ts` (GET list + POST) and `complaints/[id]/route.ts` (PUT); `Complaint.confidential` column + migration. (Req 9.2 `GET /api/complaints/[id]` remains open.)
9. ~~**MFA per-role** (Req 1.1): add a per-role MFA policy (require MFA for privileged roles regardless of email presence).~~ **Closed (2026-08-04)** via `MFA_REQUIRED_ROLES` in `src/lib/constants.ts` + per-role MFA gate in `src/app/api/auth/login/route.ts:421-448`; `requireReauth` added to `admin/hrims-settings/route.ts` PUT (Req 14.2). Tests in `src/app/api/auth/login/mfa-role.test.ts`. ~~Self-service token-based reset flow (Req 1.10)~~ **Closed (2026-07-25)** — see row 1.10.
...
> **Closed in this pass (2026-07-25):** per-user sliding login rate limit (Req 1.8) — `checkRateLimitSliding` + `buildUserRateLimitKey` in `src/lib/rate-limiter.ts`, wired into `src/app/api/auth/login/route.ts` ahead of the DB lookup; unit tests in `src/lib/rate-limiter-sliding.test.ts` and route tests in `src/app/api/auth/login/route.test.ts`. Real-time security alerting (Req 26.6, also closes 23.6's outbound gap) — `src/lib/security-alerts.ts` (`dispatchSecurityAlert`: webhook/SIEM + email, severity-gated, fire-and-forget, channel-isolated) wired into `logAuditEvent`; env config in `.env.example`/`.env.production`; tests in `src/lib/security-alerts.test.ts` + `src/lib/audit-logger.alert-wiring.test.ts`. Generic file download/preview/exists IDOR (Req 10.1–10.2, 17.3, 27.1, 30.1) — `src/lib/file-access.ts` (`checkFileAccess`/`authorizeFileOrDeny`: objectKey → owner → role matrix, fail-closed for at-risk roles, complaint-relationship grant) wired before any MinIO access in `files/download`, `files/preview`, `files/exists`; tests in `src/lib/file-access.test.ts` + `src/app/api/files/files-access.route.test.ts`. File integrity fail-closed for sensitive keys (Req 10.6) — `src/lib/file-integrity.ts` `isSensitiveObjectKey` + `VerifyOptions.failClosed`: `verifyFileHash`/`verifyDocumentHash` now block (`ok=false`, new `lookup_failed` reason) and emit a CRITICAL `POTENTIAL_BREACH` audit (`NO_HASH_RECORDED`/`INTEGRITY_LOOKUP_FAILED`) when a sensitive object key (`employee-documents/`*, `employee-photos/`*) has no recorded hash or the integrity-table lookup errors; non-sensitive keys keep fail-open. `files/exists` audit logging (Req 10.8) — new `EXISTS` action / `FILE_EXISTS_CHECK` event type in `logFileAction` (`src/lib/audit-logger.ts`) wired into `files/exists/[...objectKey]/route.ts` (both exists:true/false outcomes); tests in `src/lib/file-integrity.test.ts`. Malware scanning coverage + env-disable hardening (Req 10.7) — `src/lib/hrims-photo-scan.ts` (`scanEmployeePhoto`: scans `Employee.photo.content` before persisting, fail-closed drop + CRITICAL `HRIMS_PHOTO_MALWARE`/`HRIMS_PHOTO_SCAN_FAILED` audit) wired into `hrims/sync-employee/route.ts`; generic `files/upload` confirmed scanned via `validateFileUpload` step 5 (new `generic`-context tests); `clamav.ts` `isClamAVEnabled` now ignores `CLAMAV_ENABLED=false` in production without `CLAMAV_DISABLE_ALLOWED=true` (two-key gate, dev/CI escape hatch retained, logs once); tests in `clamav.test.ts`, `file-validation.test.ts`, `sync-employee/route.test.ts`. Silent file-audit `.catch(()=>{})` cleanup (Req 10.8 / Remediation #13) — new `safeAuditLog(p, context)` helper in `audit-logger.ts` (logs, never swallows) now wraps every file-domain audit write (`files/download`, `files/preview`, `files/upload`, `files/exists`, `file-access.ts` IDOR, `file-integrity.ts` CRITICAL writes) replacing the bare `.catch(()=>{})`. Silent `[id]` workflow 403s + proxy edge-blocks (Req 15.7, 17.6) — new `src/lib/workflow-access.ts` (`denyWorkflowAccess`: logs `logAccessDenied` then returns the 403/400) wired into every institution-ownership / self-approval / role-gating / submitter-only / FSM-transition denial on all 8 `[id]` workflow routes (`promotions`, `confirmations`, `lwop`, `termination`, `retirement`, `cadre-change`, `resignation`, `service-extension`) PUT/PATCH/GET/DELETE; `src/proxy.ts` `logUnauthorizedAttempt` now fires `logUnauthorizedAccess` (unauthenticated) / `logForbiddenRoute` (authenticated wrong-role) to the audit trail in addition to console; tests in `workflow-access.test.ts` + `proxy.test.ts`. Self-service token-based password reset (Req 1.10) — new hashed `PasswordResetToken` table + `src/lib/password-reset.ts` (`resolveResetToken`/`consumePasswordResetToken` race-safe single-use, `incrementResetVerifyAttempts` cap) + `POST /api/auth/forgot-password` (non-enumerable, per-identifier sliding limit) + `POST /api/auth/reset-password` (full change-password validation chain, atomically consumes token, clears standard lockout flags, preserves admin `isManuallyLocked`, invalidates all sessions, `PASSWORD_RESET` audit) + `sendPasswordResetEmail` + `/forgot-password` & `/reset-password` pages + login "Forgot password?" link; env `PASSWORD_RESET_TOKEN_EXPIRY_MINUTES`/`PASSWORD_RESET_MAX_VERIFY_ATTEMPTS` in `.env.example`/`.env.production`; tests in `src/lib/password-reset.test.ts`, `forgot-password/route.test.ts`, `reset-password/route.test.ts`. Complainant-identity confidentiality (Req 9.6 / Remediation #8) — new `Complaint.confidential` column + migration `20260725020000_add_complaint_confidential`; `src/lib/complaint-confidentiality.ts` (`canSeeComplainantIdentity` need-to-know rule + `redactComplainantIdentity` + `resolveConfidential`) wired into `complaints/route.ts` GET (redact `employeeId`/`employeeName`/`zanId`/phones + `confidential`/`complainantIdentityRedacted` flags) and POST (store `confidential`; harassment/"Unyanyasaji" always confidential; redacted name in officer notifications/emails for confidential complaints) and `complaints/[id]/route.ts` PUT (officer-writable `confidential` with harassment-lock + response redaction); frontend confidentiality banner in `complaints/page.tsx`; tests in `src/lib/complaint-confidentiality.test.ts` (16 cases).

---

> **Closed in this pass (2026-07-26):** fuzzy employee duplicate detection (Req 6.5) — new `src/lib/employee-duplicate-detection.ts` (`normalizeName`/`levenshtein`/`nameSimilarity`/`dobDayBounds`/`findFuzzyDuplicate`/`isWithinFileFuzzyDuplicate`, zero-dependency) complements the exact-key uniqueness checks (6.2–6.4) with a name + DOB + institutionId match: the DB query is scoped to `institutionId` + the DOB calendar day, then normalized Levenshtein name similarity ≥ `DUPLICATE_NAME_SIMILARITY_THRESHOLD` (0.85) flags a duplicate. Wired into `manual-entry/route.ts` (409 block surfacing the existing record's zanId) and `bulk-upload/route.ts` (within-file fuzzy pass + per-row DB fuzzy check, both reported as per-row errors); tests in `src/lib/employee-duplicate-detection.test.ts` (26 cases), `src/app/api/employees/manual-entry/route.test.ts`, and `src/app/api/employees/bulk-upload/route.test.ts`. Req 6 moves from 5/2/1 to 6/2/0; totals 130/53/23.


---

> **Closed in this pass (2026-08-04):** Per-role MFA policy (Req 1.1 ⚠️→✅, Req 14.2 ⚠️→✅) — new `MFA_REQUIRED_ROLES` constant in `src/lib/constants.ts:14` (`['Admin','CSCS','HHRMD']`); login route MFA gate (`src/app/api/auth/login/route.ts:421-448`) now computes `mfaRequiredByRole = MFA_REQUIRED_ROLES.includes(user.role)` and blocks privileged-role logins without email (403 `MFA_REQUIRED_NO_EMAIL`) instead of silently skipping MFA; non-privileged roles retain existing behavior (MFA if email, direct login if no email). `requireReauth` added to `admin/hrims-settings/route.ts:45-49` PUT handler (closes the last reauth gap). Tests in `src/app/api/auth/login/mfa-role.test.ts` (6 cases). Req 1 moves from 11/2/0 to 12/1/0; Req 14 moves from 5/3/0 to 6/2/0; totals 132/51/23. IP-ban-on-abuse (Req 1.8, 26.1 enhancement) — new `src/lib/ip-ban-utils.ts` (`banIp`/`unbanIp`/`banIpManually`/`autoUnbanExpiredIps`/`recordFailedLoginFromIp`/`recordRateLimitHit`/`getIpBanStatus`/`isIpBanned`, pure helpers `determineBanType`/`durationMinutesForBanCount`/`isBanRowActive`, Redis sliding-window counters + atomic Lua) + `IpBan` Prisma model (`prisma/schema.prisma:552-569`) + migration `20260804000000_add_ip_ban`; wired into `src/app/api/auth/login/route.ts:46-75` (banned-IP gate before DB lookup — reads Postgres, survives Redis outage) and `src/lib/rate-limiter.ts:371` (auth-tier 429 → `recordRateLimitHit`); admin routes `src/app/api/admin/ban-ip/route.ts` (step-up reauth, Admin-only, Zod) + `src/app/api/admin/unban-ip/route.ts` + `src/app/api/admin/ip-bans/route.ts` (list) + public `src/app/api/auth/ip-ban-status/route.ts`; five new audit event types in `audit-logger.ts` (`IP_BANNED`/`IP_BANNED_UPGRADED`/`IP_AUTO_UNBANNED`/`ADMIN_IP_BAN`/`ADMIN_IP_UNBAN`); env-overridable thresholds (`IP_FAILED_LOGIN_BAN_THRESHOLD`/`IP_RATELIMIT_HIT_BAN_THRESHOLD`/`IP_BAN_WINDOW_SECONDS`/`IP_TRUSTED_IPS`) documented in `.env.example`/`.env.production`; tests in `src/lib/ip-ban-utils.test.ts`, `src/lib/rate-limiter.ip-ban.test.ts`, `src/app/api/auth/login/ip-ban.test.ts`, `src/app/api/admin/ban-ip.route.test.ts`, `src/app/api/admin/ip-bans.route.test.ts`. This is an additive enhancement to already-✅ controls (Req 1.8 rate limiting, Req 26.1 failed-login monitoring) — no verdict changes from the IP-ban feature itself.

---

*Originally prepared from codebase analysis of `/home/latest` (branch `fix/e2e-chronic-failures`) on 2026-07-18. Re-audited and updated 2026-07-21, 2026-07-26, and 2026-08-04 against the current working tree; all `file:line` references reflect the current code. Each table row maps directly to one or more UAT/penetration test cases per the implementation notes in `CSMS_Security_Requirements_Transform.md`. Controls that are tighter than the specified threshold are scored ✅ and are intentionally not loosened.*